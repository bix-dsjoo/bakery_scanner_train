...
DINO, SAM 2와 detector ensemble은 상시 실행하지 않는다. Classifier ensemble도
E6의 선택적 고비용 ablation에서만 다룬다. 기본 cascade가 정확도 목표를 달성하지
못하고 해당 구성의 개선 효과가 동일 train-side split에서 확인될 때만 승격한다.
...
새 crop과 hard negative는 `datasets/derived/` 아래에, 합성 장면은 정확히
`datasets/derived/synthetic/` 아래에 원자적으로
...
Holdout 동결은 두 단계로 수행한다. 먼저 경로 문자열과 split 할당, seed, config
hash만으로 불변 assignment lock을 원자적으로 게시한다. 그 잠금이 게시된 뒤에만
자동화된 integrity 검사가 holdout 파일을 hash/stat/decode할 수 있다. 이 검사는
SHA-256, 크기, 채널 유효성, 파일 수만 기록하며 픽셀, 예측, score 또는 성능 지표를
노출하지 않는다. 이 문서에서 "holdout 접근"은 모델 추론, 사람이 이미지를 열어
보는 행위, 성능 계산처럼 선택에 영향을 줄 수 있는 semantic access를 뜻한다.
Assignment lock 이전의 holdout byte access와 one-shot 평가 이전의 semantic access는
오류다. Integrity 실패 시 동일 run을 덮어쓰거나 재선택하지 않고 실패 상태를
보존한 채 새 run name으로 원인을 수정한다.
...
Warm-up은 통계에서 제외하고 평균/P50/P95의 반복 횟수, CPU 모델, thread 수, 입력
크기와 batch 크기를 결과 artifact에 함께 기록한다.
...
- holdout byte integrity 검사 전에 assignment lock이 없음
- one-shot holdout semantic access 전에 freeze manifest와 one-shot lock이 없음
