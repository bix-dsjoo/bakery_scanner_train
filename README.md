...
현재 저장소에는 데이터셋 무결성 검사, COCO 검증, 학습 경로 안전장치, scene 단위 split과 detector용 합성 장면 생성·replay 검증 기반이 구현되어 있습니다. 모델 학습·추론 코드는 아직 구현되지 않았습니다.
...
Pillow는 이미지 검증과 합성 장면 생성에 사용합니다. 모델 학습 라이브러리는 아직 포함하지 않습니다.

## 합성 장면 생성과 검증

`bakery-synthetic`는 등록된 단일 객체 이미지를 명시적으로 지정한 빈 트레이 배경에 배치합니다. 배경 디렉터리는 빵이 없는 학습용 이미지만 포함해야 하며 `datasets/base/test`와 `datasets/incremental/test` 또는 그 하위 경로는 즉시 거부됩니다. 저장소에는 현재 빈 트레이 배경 자산이 포함되어 있지 않으므로 먼저 별도의 학습용 배경 디렉터리를 준비해야 합니다.
...
bakery-synthetic generate `
  ...
  --background-dir path/to/train_backgrounds `
...
```powershell
python -m bakery_scanner.synthetic_cli generate --dataset-root datasets --background-dir path/to/train_backgrounds --run-name base_seed42 --seed 42 --scene-count 100 --objects-per-scene 5
```
