...
Base 추론 재설계 실험은 개발 scene ID `0503`, `0509`와 cycle-holdout ID
`0510`을 고정합니다. 개발 배경은 `tray_white_square.png`,
`tray_wood_black_surface.png`이고 cycle-holdout 배경은
`tray_wood_white_surface.png`이며 seed는 `42`, `43`, `44`입니다. 다음 명령은
이 assignment를 immutable lock과 hash-bound manifest로 동결하고 재검증합니다.
...
이 명령에는 overwrite 모드가 없습니다. 동결 과정은 assignment lock을 먼저
게시한 뒤 무결성 확인에 필요한 hash와 이미지 decode만 수행하며, 같은 run 이름은
성공·실패 여부와 무관하게 다시 사용하지 않습니다. 현재 재설계를 위한 모델 학습
명령은 아직 구현되어 있지 않습니다.

`0510`과 세 배경은 이전 프로젝트 산출물에 영향을 준 적이 있어 이 cycle-holdout은
한 번도 관찰되지 않은 독립 test가 아닙니다. Manifest 동결은 평가 전용 test 데이터나
holdout metric 접근을 허가하지 않습니다. 후속 실험은 manifest를 먼저 검증하고,
holdout 입력을 보았던 기존 프로젝트 checkpoint가 아니라 일반 사전학습 가중치에서
시작해야 합니다.
